...
class Tapper:
    def __init__(self, tg_client: Client):
        ...
        self.skip = [
            "register_on_cex_io",
            "boost_telegram",
            "play_piggypiggy_tap_game",
            "join_btc_garden_twitter",
            "subscribe_crypto_garden_telegram",
            "subscribe_telegram",
            "join_cedex_tap_game",
            "join_wigwam_drum_game",
        ]
        self.card1 = None
        self.potential_card = {}
        self.multi = 1000000
    ...
    async def get_tg_web_data(self, proxy: str | None) -> str:
        ...
        try:
            ...
            self.first_name = tg_web_data.split('"first_name":"')[1].split(
                '","last_name"'
            )[0]
            self.last_name = tg_web_data.split('"last_name":"')[1].split(
                '","username"'
            )[0]
    ...
    async def get_user_info(self, http_client: aiohttp.ClientSession, authToken):
        ...
        if response.status == 200:
            ...
            logger.error(
                f"Error while getting user data. Response {response.status}. Try again after 30s"
            )
            await asyncio.sleep(30)
    ...
    async def tap(self, http_client: aiohttp.ClientSession, authToken, taps):
        ...
        if response.status == 200:
            ...
            if int(taps) != 0:
                logger.info(
                    f"{self.session_name} | Tapped <cyan>{taps}</cyan> times | Coin balance: <cyan>{data_response['balance_USD']}</cyan>"
                )
        else:
            json_response = await response.json()
            if "too slow" in json_response["data"]["reason"]:
                logger.error(
                    f"{self.session_name} | <red> Tap failed - please stop the code and open the bot in telegram then tap 1-2 times and run this code again. it should be worked!</red>"
                )
            else:
                print(json_response)
                logger.error(
                    f"{self.session_name} | <red> Tap failed - response code: {response.status}</red>"
                )
    ...
    async def claim_crypto(self, http_client: aiohttp.ClientSession, authToken):
        ...
        if response.status == 200:
            ...
            logger.error(
                f"{self.session_name} | <red>Claim BTC failed - response code: {response.status}</red>"
            )
    ...
    async def getConvertData(self, http_client: aiohttp.ClientSession, authToken):
        ...
        if response.status == 200:
            ...
            logger.error(
                f"{self.session_name} | <red> Can convert !| Error code: {response.status}"
            )
            return None
    ...
    async def convertBTC(self, http_client: aiohttp.ClientSession, authToken):
        ...
        if price:
            ...
            if response.status == 200:
                ...
                logger.error(
                    f"{self.session_name} | <red>Error code {response.status} While trying to convert...</red>"
                )
    ...
    async def claim_pool(self, http_client: aiohttp.ClientSession, authToken):
        ...
        if response.status == 200:
            ...
            logger.error(
                f"{self.session_name} | <red>Error code {response.status} While trying to claim from pool</red>"
            )
    ...
    async def fetch_data(self, http_client: aiohttp.ClientSession, authToken):
        ...
        if response.status == 200:
            ...
            logger.error(
                f"{self.session_name} | <red>Error code {response.status} While trying to get data</red>"
            )
    ...
    async def getUserTask(self, http_client: aiohttp.ClientSession, authToken):
        ...
        if response.status == 200:
            ...
            logger.error(
                f"{self.session_name} | <red>Error code {response.status} While trying to get completed task</red>"
            )
            return None
    ...
    async def claimTask(self, http_client: aiohttp.ClientSession, authToken, taskId):
        ...
        if response.status == 200:
            ...
            logger.error(
                f"{self.session_name} | <red>Failed to claim {taskId}. Response: {response.status}</red>"
            )
    ...
    async def checkTask(self, http_client: aiohttp.ClientSession, authToken, taskId):
        ...
        if response.status == 200:
            ...
            logger.error(
                f"{self.session_name} | <red>Failed to check task {taskId}. Response: {response.status}</red>"
            )
    ...
    async def startTask(self, http_client: aiohttp.ClientSession, authToken, taskId):
        ...
        if response.status == 200:
            ...
            logger.error(
                f"{self.session_name} | <red>Failed to start task {taskId}. Response: {response.status}</red>"
            )
            ...
            for card in category["upgrades"]:
                ...
                if card["upgradeId"] in self.card1:
                    ...
                    if len(card["levels"]) > 0:
                        potential = (
                            card["levels"][card_lvl][0] / card["levels"][card_lvl][2]
                        )
    ...
    async def buyUpgrade(self, http_client: aiohttp.ClientSession, authToken, Buydata):
        ...
        if response.status == 200:
            ...
            logger.success(
                f"{self.session_name} | <green>Successfully upgraded <blue>{Buydata['upgradeId']}</blue> to level <blue>{Buydata['nextLevel']}</blue></green>"
            )
            return True
        else:
            logger.error(
                f"{self.session_name} | <red>Error while upgrade card {Buydata['upgradeId']} to lvl {Buydata['nextLevel']}. Response code: {response.status}</red>"
            )
            return False
    ...
    async def run(self, proxy: str | None) -> None:
        ...
        while True:
            try:
                if (
                    time() - access_token_created_time >= token_live_time
                    or authToken == ""
                ):
                ...
                if (
                    settings.AUTO_CONVERT
                    and self.btc_balance >= settings.MINIMUM_TO_CONVERT
                ):
                ...
                if settings.AUTO_BUY_UPGRADE:
                    ...
                    for card in sorted_potential_card:
                        if self.checkDependcy(
                            sorted_potential_card[card]["dependency"]
                        ):
                            if int(sorted_potential_card[card]["cost"]) <= int(
                                round(float(self.coin_balance))
                            ):
                                check = await self.buyUpgrade(
                                    http_client, authToken, sorted_potential_card[card]
                                )
                                if check:
